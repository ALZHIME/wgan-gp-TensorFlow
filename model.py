...
def buildGenerator(z, z_dim, img_size, nBatch, reuse=False, isTraining=True):
    with tf.variable_scope("Generator") as scope:
        ...
        h = z
        # fc1
        g_fc1_w, g_fc1_b = _fc_variable([z_dim,512*img_size//2**4*img_size//2**4],name="fc1")
        h = tf.matmul(h, g_fc1_w) + g_fc1_b
        h = tf.nn.relu(h)
        #
        h = tf.reshape(h,(nBatch,img_size//2**4,img_size//2**4,512))
        # deconv4
        ...
        # deconv1
        g_deconv1_w, g_deconv1_b = _deconv_variable([5,5,64,3],name="deconv1")
        h = _deconv2d(h,g_deconv1_w, output_shape=[nBatch,img_size,img_size,3], stride=2) + g_deconv1_b
        y = tf.tanh(h)
